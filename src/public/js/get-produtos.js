...
async function pesquisaProdutos (result, type) {
  if (type == '') {
    const retorno = await fetch('http://88.0.1.6:8000/api/products/categoria/' + result, {
    ...
    if (result == ''){ 
      const retorno = await fetch('http://88.0.1.6:8000/api/products')
      const produtos = await retorno.json()
      preencheTela(produtos) 
    } else {
      const retorno = await fetch('http://88.0.1.6:8000/api/products/' + type + '/' + result)
      const produtos = await retorno.json()
      preencheTela(produtos)
    }
  }  
...
async function pesquisaProdutoUni (result) {
  const retorno = await fetch('http://88.0.1.6:8000/api/products/codigo/' + result)
  const produto = await retorno.json()
  preencheTelaUni(produto) 
}

async function listaCategorias () {
  const retorno = await fetch('http://88.0.1.6:8000/api/products/categorias')
