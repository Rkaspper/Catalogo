...
async function pesquisaProdutos (result, type) {
  if (type == '') {
    const retorno = await fetch('https://catalogo-aluita.onrender.com/api/products/categoria/' + result, {
    ...
    if (result == ''){ 
      ...
      const retorno = await fetch('https://catalogo-aluita.onrender.com/api/products/' + type + '/' + result)
      const produtos = await retorno.json()
      preencheTela(produtos)
    }
  }  
...
async function pesquisaProdutoUni (result) {
  const retorno = await fetch('https://catalogo-aluita.onrender.com/api/products/codigo/' + result)
  const produto = await retorno.json()
  preencheTelaUni(produto) 
}

async function listaCategorias () {
  const retorno = await fetch('https://catalogo-aluita.onrender.com/api/products/categorias')
